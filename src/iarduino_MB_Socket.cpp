...
bool	iarduino_MB_Socket::setState(bool f){																				//	f - состояние розетки (true/false)
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return false; }												//	Ошибка синтаксиса, модуль не инициализирован.
			return MB_writeDO(DO_RELAY, f);																					//	Записываем f в регистр "Coil" DO[DO_RELAY].
}																															//
																															//
...
int8_t	iarduino_MB_Socket::getState(void){																					//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return false; }												//	Ошибка синтаксиса, модуль не инициализирован.
			return MB_readDO(DO_RELAY);																						//	Читаем регистр "Coil" DO[DO_RELAY]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
}																															//
																															//
...
int8_t	iarduino_MB_Socket::digitalRead(uint8_t num){																		//	num - один из вариантов SOCKET_SENSOR_XXX.
			...
			//	Читаем состояние всех датчиков одним запросом:																//
				if( MB_requestFrom(DISCRETE_INPUTS, DI_MIN, 3) != 3 ){ return -1; }											//	Читаем регистры "Discrete Inputs" начиная с регистра DI[DI_MIN], всего 3 регистра. Функция возвращает количество прочитанных значений, или 0 при неудаче.
			...
			if( num==SOCKET_SENSOR_MIN ){ return MB_readDI(DI_MIN); }else													//	Читаем регистр "Discrete Input" DI[DI_MIN]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==SOCKET_SENSOR_MID ){ return MB_readDI(DI_MID); }else													//	Читаем регистр "Discrete Input" DI[DI_MID]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==SOCKET_SENSOR_MAX ){ return MB_readDI(DI_MAX); }else													//	Читаем регистр "Discrete Input" DI[DI_MAX]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
...
int8_t	iarduino_MB_Socket::connectRead(uint8_t num){																		//	num - один из вариантов SOCKET_SENSOR_XXX.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			int32_t result = MB_diagnostic(2);																				//	Выполняем команду диагностики, функция 0x0002 возвращает значение регистра диагностики.
...
bool	iarduino_MB_Socket::enableWDT(uint16_t ms){																			//	ms - время в миллисекундах от 1 до 65535.
			...
			if( !MB_writeAO(AO_WDT, ms) ){ return false; }																	//	Записываем значение ms в регистр "Holding Register" AO[AO_WDT]. Функция возвращает 1 при успехе, 0 при неудаче.
			return MB_writeDO(DO_WDT, 1);																					//	Устанавливаем регистр "Coil" DO[DO_WDT]. Функция возвращает 1 при успехе, 0 при неудаче.
}																															//
																															//
...
bool	iarduino_MB_Socket::disableWDT(void){																				//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return false; }												//	Ошибка синтаксиса, модуль не инициализирован.
			return MB_writeDO(DO_WDT, 0);																					//	Сбрасываем регистр "Coil" DO[DO_WDT]. Функция возвращает 1 при успехе, 0 при неудаче.
}																															//
																															//
...
bool	iarduino_MB_Socket::resetWDT(void){																					//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return false; }												//	Ошибка синтаксиса, модуль не инициализирован.
			return MB_writeAO(AO_WDT, valWDT);																				//	Записываем значение valWDT в регистр "Holding Register" AO[AO_WDT]. Функция возвращает 1 при успехе, 0 при неудаче.
}																															//
																															//
...
int8_t	iarduino_MB_Socket::getStateWDT(void){																				//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			return MB_readDO(DO_WDT);																						//	Читаем регистр "Coil" DO[DO_WDT]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
}																															//
																															//
/**		ФУНКЦИИ ДУБЛЁРЫ ОБЪЕКТА objModbus ДЛЯ ЧТЕНИЯ/ЗАПИСИ С НЕСКОЛЬКИХ ПОПЫТОК	**/										//
																															//
int8_t		iarduino_MB_Socket::MB_readDO		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int8_t   i=objModbus->coilRead            (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
int8_t		iarduino_MB_Socket::MB_readDI		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int8_t   i=objModbus->discreteInputRead   (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
int32_t		iarduino_MB_Socket::MB_readAO		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int32_t  i=objModbus->holdingRegisterRead (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
int32_t		iarduino_MB_Socket::MB_readAI		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int32_t  i=objModbus->inputRegisterRead   (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
uint16_t	iarduino_MB_Socket::MB_requestFrom	(uint8_t  type, uint16_t reg, uint16_t n){ uint8_t cntRW=maxRW; while(cntRW){ uint16_t i=objModbus->requestFrom         (valID, type,reg,n); if( i> 0 ){ return i;    }else{ cntRW--; } } return  0;    }
uint8_t		iarduino_MB_Socket::MB_getInfo		(uint8_t  id							){ uint8_t cntRW=maxRW; while(cntRW){ uint8_t  i=objModbus->getInfo             (id);                if( i> 0 ){ return i;    }else{ cntRW--; } } return  0;    }
int32_t		iarduino_MB_Socket::MB_diagnostic	(uint16_t func, uint16_t data			){ uint8_t cntRW=maxRW; while(cntRW){ int32_t  i=objModbus->diagnostic          (valID, func, data); if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
bool		iarduino_MB_Socket::MB_changeID		(uint8_t  newID							){ uint8_t cntRW=maxRW; while(cntRW){        if( objModbus->changeID            (valID, newID)  ){ valID=newID;  return true; }else{ cntRW--; } } return false; }
bool		iarduino_MB_Socket::MB_writeDO		(uint16_t reg, bool val					){ uint8_t cntRW=maxRW; while(cntRW){        if( objModbus->coilWrite           (valID, reg, val)             ){ return true; }else{ cntRW--; } } return false; }
bool		iarduino_MB_Socket::MB_writeAO		(uint16_t reg, uint16_t val				){ uint8_t cntRW=maxRW; while(cntRW){        if( objModbus->holdingRegisterWrite(valID, reg, val)             ){ return true; }else{ cntRW--; } } return false; }
